...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const message = await interaction.reply({ content: 'Please choose your character:', components: [row], ephemeral: true, fetchReply: true });
            
            // Set up collector for timeout handling
            ...
            collector.on('end', (collected, reason) => {
                if (reason === 'time') {
                    interaction.editReply({ content: '⏰ Character selection timed out.', components: [] }).catch(() => {});
                }
            });
        } catch (error) {
